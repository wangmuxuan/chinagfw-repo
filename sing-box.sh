...
# Download helper (stdout variant) with mirror/proxy fallback.
download_stdout_with_fallback() {
  # Usage: download_stdout_with_fallback URL1 URL2 URL3...
  local url i
  for url in "$@"; do
    i=1
    while [ "$i" -le 3 ]; do
      if command -v curl >/dev/null 2>&1; then
        curl -fsL --connect-timeout 10 --max-time 180 \
          --retry 2 --retry-delay 1 --retry-all-errors "$url" && return 0
      elif command -v wget >/dev/null 2>&1; then
        wget --no-check-certificate --tries=3 --timeout=180 -qO- "$url" && return 0
      else
        return 127
      fi
      i=$((i + 1))
...
# Self-update / rerun via China-friendly mirror with fallback.
bash <(download_stdout_with_fallback \
  "${MIRROR_BASE}/sing-box.sh" \
  "https://ghproxy.com/https://raw.githubusercontent.com/fscarmen/sing-box/main/sing-box.sh" \
